...
<script>
  ...
  fromDate.addEventListener("change", function() {
    ...
    // Auto Suggest partners ================================================
    $(document).ready(function () {
    // Function to initialize autocomplete for participant name fields
    function initAutocomplete() {
        $(".participantName").autocomplete({
            source: function (request, response) {
                // Get current participant names to filter out duplicates
                let existingNames = $('.participantName').map(function() {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        // Filter out already existing names from the suggestions
                        const filteredData = data.filter(item => !existingNames.includes(item.name));
            ...
            select: function (event, ui) {
                ...
                // Focus the EPF input field after selection
                $row.find('.epf').focus();
    ...
    // Function to check for duplicate EPF numbers
    function hasDuplicateEPF() {
        let epfNumbers = [];
        let hasDuplicate = false;
        
        $('.epf').each(function() {
            let epfValue = $(this).val().trim();
            if (epfValue && epfNumbers.includes(epfValue)) {
                hasDuplicate = true;
                return false; // Exit loop early
            }
            epfNumbers.push(epfValue);
        });
    ...
    function hasDuplicateNames() {
        ...
        $('.participantName').each(function() {
            let nameValue = $(this).val().trim();
            if (nameValue && names.includes(nameValue)) {
                hasDuplicate = true;
                return false; // Exit loop early
            }
    ...
    window.addParticipantRow = function() {
        ...
        // Add event listener to check for duplicate values when typing
        $('.epf').off('input').on('input', function() {
            if (hasDuplicateEPF()) {
                alert("Duplicate EPF number detected. Please enter a unique EPF number.");
                $(this).val(''); // Clear the duplicate entry
            }
        });

        $('.participantName').off('input').on('input', function() {
            if (hasDuplicateNames()) {
                alert("Duplicate participant name detected. Please enter a unique name.");
                $(this).val(''); // Clear the duplicate entry
            }
        });
    }
