...
$sql = "SELECT transport.*,employees.name FROM transport JOIN employees ON transport.request_user = employees.id WHERE employees.division='$division' ORDER BY transport.id DESC";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $requestHistory[] = [
            'request_date'=>'  from ' .date("h:i A", strtotime( $row['from_time'])).' on '. $row['request_date'] . ' - to ' . date("h:i A", strtotime( $row['to_time'])) .' on '. $row['to_date'] ,
